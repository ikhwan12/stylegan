...
def main():
    ...
    for images_batch in tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size):
        ...
        if args.output_video:
          video_out = {}
          for name in names:
            video_out[name] = cv2.VideoWriter(os.path.join(args.video_dir, '{}.avi'.format(name)),cv2.VideoWriter_fourcc(*args.video_codec), args.video_frame_rate, (args.video_size,args.video_size))

        perceptual_model.set_reference_images(images_batch)
        dlatents = None
        if (args.load_last != ''): # load previous dlatents for initialization
            for name in names:
                dl = np.expand_dims(np.load(os.path.join(args.load_last, '{}.npy'.format(name))),axis=0)
                if (dlatents is None):
                    dlatents = dl
                else:
        ...
        for img_array, dlatent, img_path, img_name in zip(generated_images, generated_dlatents, images_batch, names):
            ...
            img.save(os.path.join(args.generated_images_dir, '{}.png'.format(img_name)), 'PNG')
            np.save(os.path.join(args.dlatent_dir, '{}.npy'.format(img_name)), dlatent)
